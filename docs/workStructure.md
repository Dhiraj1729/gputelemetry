# Work structure and development history

The current installation guide is [README](../README.md). This file preserves the previous day-by-day README, including historical status statements and prototype commands; those statements are not the current release status. See [submission audit](SUBMISSION_AUDIT.md) for current evidence and remaining gates.

## Repository map

| Path | Responsibility |
| --- | --- |
| `cmd/` | Executables: streamer, queue, collector, API, migrate, OpenAPI generator, diagnostic consumer |
| `internal/` | Configuration, logging, telemetry model, producer, broker, queue client, collector, PostgreSQL and HTTP API |
| `migrations/` | Embedded PostgreSQL schema migrations |
| `api/openapi.yaml` | Generated API contract |
| `deploy/docker/` | Shared multi-stage Dockerfile |
| `deploy/helm/gpu-telemetry/` | Kubernetes packaging |
| `scripts/` | Environment, image and deployment workflows |
| `tests/` | Process, PostgreSQL and Helm packaging tests |
| `Problemstatement/*.csv` | Included dummy telemetry input |

## Historical README

# Elastic GPU Telemetry Pipeline

**Day 5 packaging added:** five Docker targets and one Helm chart with database bootstrap, migration hooks, persistent storage and local verification scripts. See [Day 5 runbook](DAY5_PACKAGING.md) and [verification status](DAY5_VERIFICATION.md). Runtime image/deployment checks are pending access to the Mac Docker/Minikube environment.

**Day 4 API added:** read-only Go/Huma API, inclusive telemetry queries, generated OpenAPI, health/readiness and bounded resource use. See [Day 4 API guide](DAY4_API.md) and [verification status](DAY4_VERIFICATION.md).

**Day 3 collector added:** production Go collector, PostgreSQL persistence/migrations, commit-before-ACK, bounded workers/pool and health endpoints. See [Day 3 setup and design](DAY3_COLLECTOR.md) for database setup, configuration, run commands and tests. The Day 4 API is now implemented; real database verification status is recorded separately.
...
See [Day 2 protocol](DAY2_PROTOCOL.md) for full wire schemas, status codes, retry windows, clock assumptions, storage accounting, cleanup and shutdown behavior.
